...
if st.session_state.dark_mode:
    ...
    RED      = "#ef4444"
    NAV_BG   = "#0f172a"
    CARD_SH  = "0 1px 4px rgba(0,0,0,0.3)"
    PLOT_BG  = "#0f172a"
    PLOT_PAP = "#0a0e1a"
    GRID_COL = "#1e293b"
    INFO_BG  = "rgba(56,189,248,0.08)"
    INFO_BOR = "rgba(56,189,248,0.2)"
    INFO_COL = "#93c5fd"
    WARN_BG  = "rgba(251,191,36,0.08)"
    WARN_BOR = "rgba(251,191,36,0.2)"
    WARN_COL = "#fbbf24"
    OK_BG    = "rgba(34,197,94,0.08)"
    OK_BOR   = "rgba(34,197,94,0.2)"
    OK_COL   = "#86efac"
    TAB_ACT  = "#1e40af"
    BTN_BG   = "linear-gradient(135deg,#1d4ed8,#4f46e5)"
    THEME_ICON = "☀️"
    ...
    RED      = "#e11d48"
    NAV_BG   = "#ffffff"
    CARD_SH  = "0 1px 4px rgba(0,0,0,0.05)"
    PLOT_BG  = "#fafafa"
    PLOT_PAP = "#ffffff"
    GRID_COL = "#f0f1f5"
    INFO_BG  = "#eff6ff"
...
.kpi-label {{ color: {SUBTEXT}; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 6px; }}
.kpi-value {{ color: {TEXT}; font-size: 24px; font-weight: 700; font-family: 'JetBrains Mono', monospace; letter-spacing: -0.5px; }}
.kpi-sub {{ font-size: 13px; margin-top: 4px; }}

.sec-header {{
    font-size: 11px; font-weight: 700; text-transform: uppercase;
    letter-spacing: 1.2px; color: {SUBTEXT};
...
.rec-reasoning {{ font-size: 14px; color: {SUBTEXT}; line-height: 1.7; margin-top: 12px; }}
...
with col_nav1:
    st.markdown(f"""
    <div class="navbar">
        <div class="nav-logo">Stock<span>AI</span> 📈</div>
...
def chart_layout(title="", height=400):
    return dict(
        title=dict(text=title, font=dict(color=SUBTEXT, size=13, family="Inter")),
        height=height,
        paper_bgcolor=PLOT_PAP,
        plot_bgcolor=PLOT_BG,
        font=dict(color=SUBTEXT, family="Inter"),
...
with tab1:
    ...
    if not cache:
        ...
        for col, label, val in [
            ...
            with col:
                st.markdown(f'<div class="kpi-card"><div class="kpi-label">{label}</div><div class="kpi-value" style="font-size:17px">{val}</div></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="sec-header">AI Recommendations</div>', unsafe_allow_html=True)
        for rec in recs:
                ...
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
                    <div style="display:flex;align-items:center;gap:16px;">
                        ...
                        <div>
                            ...
                            <div class="rec-sector">🏭 {rec.get('sector','')}</div>
                        </div>
                    </div>
            ...
            for i, s in enumerate(all_scores):
                ...
                st.markdown(f"""<div class="stock-row">
                    ...
                    <div style="display:flex;gap:18px;align-items:center;">
                        ...
                        <span style="color:{SUBTEXT};font-size:12px;">Score {s['tech_score']}</span>
                        <span style="color:{sc};font-weight:700;font-size:13px;">{sig}</span>
                    </div>
                </div>""", unsafe_allow_html=True)

        st.markdown('<div class="sec-header">Refresh (Every 3 Months)</div>', unsafe_allow_html=True)
        st.markdown('<div class="warn-pill">⏱ ~$0.30 per scan · 8-10 mins · 100 stocks</div>', unsafe_allow_html=True)
        if st.button("🔄 Run New Scan"):
            with st.spinner("Scanning 100 stocks..."):
                from scanner import run_monthly_scan
                run_monthly_scan()
            st.markdown('<div class="ok-pill">✅ Done! Reload page to see new results.</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if st.button("Analyze Stock"):
        ...
        for col, label, val, extra in [
            (c1, "Price", f"${curr:.2f}", ""),
            (c2, "RSI (14)", f"{rsi_v:.1f}" if rsi_v else "N/A",
        ...
        with cr:
            ...
            fig_rsi.add_trace(go.Scatter(x=df.index, y=df['RSI'], name='RSI',
                line=dict(color='#8b5cf6', width=2), fill='tozeroy', fillcolor='rgba(139,92,246,0.06)'))
...
# ══════════════════════════════
with tab4:
    st.markdown('<div class="info-pill">💡 <strong>Weekly SIP Strategy</strong> — Fixed weekly investment split across top 5 AI picks. Watch it compound.</div>', unsafe_allow_html=True)
    ...
    for col, market, symbol, currency in [(col1,"US","$","USD"),(col2,"IN","₹","INR")]:
        with col:
            ...
            if perf["total_invested"] > 0:
                    ...
                    st.markdown(f"""<div class="holding-row">
                        <div style="display:flex;justify-content:space-between;align-items:center;">
                            <div>
                                ...
                                <div class="h-detail">{h['shares']} sh · avg {symbol}{h['avg_buy_price']}</div>
                            </div>
                ...
                st.markdown(f'<div class="info-pill">No investments yet. Add your first {symbol}100 below.</div>', unsafe_allow_html=True)
            ...
            if top_picks:
                st.markdown(f'<div class="info-pill">Splits equally across: <strong>{", ".join(top_picks)}</strong></div>', unsafe_allow_html=True)
            else:
            ...
            if st.button(f"💰 Invest {symbol}{int(amount)}", key=f"btn_{market}", type="primary"):
                if top_picks:
                    ...
                    st.markdown('<div class="warn-pill">Run scanner first.</div>', unsafe_allow_html=True)
    ...
    if us_inv or in_inv:
        ...
        if us_inv:
            ...
            fig.add_trace(go.Scatter(x=dates, y=cum, name="US ($)", line=dict(color=ACCENT, width=2.5), fill='tozeroy', fillcolor=f'rgba(0,200,5,0.06)'))
        if in_inv:
